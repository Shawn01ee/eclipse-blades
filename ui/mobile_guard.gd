...
func _draw() -> void:
	...
	var center := size * 0.5
	# 가로로 눕힌 휴대폰과 회전 화살표.
	var phone := Rect2(center - Vector2(150, 165), Vector2(300, 160))
	draw_rect(phone, UiKit.PAPER_LIGHT)
	draw_rect(phone, UiKit.INK, false, 7.0)
	draw_circle(phone.position + Vector2(25, 80), 6, UiKit.INK)
	draw_arc(center - Vector2(0, 85), 120, -2.65, -0.45, 32, UiKit.SEAL, 8.0)
	var arrow_tip := center + Vector2(108, -138)
	...
	draw_string(f, Vector2(center.x - title_w * 0.5, center.y + 85), title,
	...
	draw_string(f, Vector2(center.x - sub_w * 0.5, center.y + 140), sub,
